...
PUF_FN bool ref_isPenaltyShootOut(Stadium *stadium, const Side side);
...
PUF_FN void kick(Stadium *stadium, int player_index, double power, double dir) {
    ...
    if (stadium->playmode == PM_BeforeKickOff ||
        stadium->playmode == PM_AfterGoal ||
        stadium->playmode == PM_OffSide ||
        stadium->playmode == PM_Illegal_Defense ||
        stadium->playmode == PM_Foul_Charge ||
        stadium->playmode == PM_Foul_Push ||
        stadium->playmode == PM_Back_Pass ||
        stadium->playmode == PM_Free_Kick_Fault ||
        stadium->playmode == PM_CatchFault ||
...
PUF_FN void changePlayMode(Stadium *stadium, const PlayMode pm, const Side pm_side) {
    stadium->playmode = pm;
    stadium->playmode_side = pm_side;
    #if REFEREES_ENABLED
    ...
    if (pm == PM_KickOff
        ...
        || pm == PM_CornerKick
        || pm == PM_GoalKick)
        stadium->kick_off_side = pm_side;
    else if (pm == PM_Illegal_Defense)
        stadium->kick_off_side = -pm_side;
...
PUF_FN void callHalfTime(Stadium *stadium, const Side kick_off_side, const int half_time_count) {
    ...
    changePlayMode(stadium, PM_BeforeKickOff, NEUTRAL);
...
PUF_FN void ref_placeBallAndChangePlayMode(Stadium *stadium, const PlayMode pm, const Side pm_side, const Side kick_off_side, float x, float y) {
    placeBall(stadium, kick_off_side, x, y);
    if (ref_isPenaltyShootOut(stadium, NEUTRAL) && (pm == PM_PlayOn || pm ==  PM_Drop_Ball)) {
        ; // never change pm to play_on in penalty mode
    }
    else
        changePlayMode(stadium, pm, pm_side);
}
...
PUF_FN void ref_clearPlayersFromBall(Stadium *stadium, const Side side) {
    ...
    const float clear_dist = (pm == PM_Illegal_Defense
        || pm == PM_Back_Pass
        || ((pm == PM_Foul_Charge || pm == PM_Foul_Push)
            && ref_inPenaltyArea(stadium->playmode_side, stadium->ball_pos_x, stadium->ball_pos_y)))
        ? GOAL_AREA_LENGTH
        : KICK_OFF_CLEAR_DISTANCE;
    const bool indirect = (pm == PM_Back_Pass
        || pm == PM_Foul_Charge
        || pm == PM_Foul_Push
        || pm == PM_IndFreeKick);
...
PUF_FN bool ref_isPenaltyShootOut(Stadium *stadium, const Side side) {
    ...
    case PM_PenaltySetup:
    case PM_PenaltyReady:
    case PM_PenaltyTaken:
    case PM_PenaltyMiss:
    case PM_PenaltyScore:
        if (stadium->playmode_side == LEFT)
            bLeft = true;
        else if (stadium->playmode_side == RIGHT)
            bRight = true;
        break;
    default:
        return false;
...
PUF_FN void ref_placePlayersInTheirField(Stadium *stadium) {
    const bool kick_off_offside = (KICK_OFF_OFFSIDE && stadium->playmode == PM_KickOff);
    for (int i = 0 ; i < NUM_PLAYERS ; i++) {
        if (!stadium->players.enable[i]) continue;
...
PUF_FN void ref_awardDropBall(Stadium *stadium, float x, float y) {
    ...
    ref_placeBallAndChangePlayMode(stadium, PM_Drop_Ball, NEUTRAL, NEUTRAL, x, y);
    placePlayersInField(stadium);

    if (!ref_isPenaltyShootOut(stadium, NEUTRAL))
        changePlayMode(stadium, PM_PlayOn, NEUTRAL);
}

PUF_FN void ref_awardFreeKick(Stadium *stadium, const Side side, float x, float y) {
    ref_truncateToPitch(&x, &y);
    ref_moveOutOfPenalty((Side)(-side), &x, &y);
    ref_placeBallAndChangePlayMode(stadium, PM_FreeKick, side, side, x, y);
}

PUF_FN void ref_awardGoalKick(Stadium *stadium, const Side side, float x, float y) {
    stadium->ball_catcher_index = -1;
    float new_y = -GOAL_AREA_WIDTH * 0.5f;
    if (y > 0.0f)
        new_y *= -1;

    x = side * (-PITCH_LENGTH * 0.5f + GOAL_AREA_LENGTH);
    ref_placeBallAndChangePlayMode(stadium, PM_GoalKick, side, side, x, new_y);
}

PUF_FN void ref_awardCornerKick(Stadium *stadium, const Side side, float x, float y) {
    stadium->ball_catcher_index = -1;
    float new_y = -PITCH_WIDTH * 0.5f + CORNER_KICK_MARGIN;
    if (y > 0.0f)
        new_y *= -1;

    x = side * (PITCH_LENGTH * 0.5f - CORNER_KICK_MARGIN);
    ref_placeBallAndChangePlayMode(stadium, PM_CornerKick, side, side, x, new_y);
}

PUF_FN void ref_awardKickIn(Stadium *stadium, const Side side, float x, float y) {
    stadium->ball_catcher_index = -1;
    ref_truncateToPitch(&x, &y);
    ref_placeBallAndChangePlayMode(stadium, PM_KickIn, side, side, x, y);
}
...
PUF_FN void time_ref_analyse(Stadium *stadium) {
    ...
    if (pm == PM_BeforeKickOff
        ...
        || pm == PM_AfterGoal
        || pm == PM_OffSide
        || pm == PM_Illegal_Defense
        || pm == PM_Foul_Charge
        || pm == PM_Foul_Push
        || pm == PM_Back_Pass
        || pm == PM_Free_Kick_Fault
        || pm == PM_CatchFault)
        return;
    ...
    if (HALF_TIME > 0) {
        ...
        if (stadium->time >= maximum_time) {
            if (PENALTY_SHOOT_OUTS && stadium->team_left_points == stadium->team_right_points)
                ...
                changePlayMode(stadium, PM_TimeOver, NEUTRAL);
                return;
            }
        }
        // overtime
...
PUF_FN void offside_ref_checkPlayerAfterOffside(Stadium *stadium) {
    if (stadium->playmode != PM_OffSide || stadium->playmode_side == NEUTRAL)
        return;

    Side offsideside = NEUTRAL;
    float center_x = 0.0f, center_y = 0.0f, size_x = 0.0f, size_y = 0.0f;
    center_x = stadium->playmode_side * (-PITCH_LENGTH / 4.0f) + stadium->offside_ref_offside_pos_x / 2.0f;
    size_x = PITCH_LENGTH / 2 + stadium->playmode_side * stadium->offside_ref_offside_pos_x;
    size_y = PITCH_WIDTH;
    offsideside = stadium->playmode_side;
    ...
    for (int i = 0 ; i < NUM_PLAYERS ; i++) {
        ...
        if (distance(c_center_x, c_center_y, stadium->players.pos_x[i], stadium->players.pos_y[i]) <= c_radius)
            nearestEdgeCircle(c_center_x, c_center_y, c_radius, 
                stadium->players.pos_x[i], stadium->players.pos_y[i], 
                &stadium->players.pos_x[i], &stadium->players.pos_y[i]);

        if (!inArea(left, right, top, bottom, stadium->players.pos_x[i], stadium->players.pos_y[i])) {
            float new_pos_x, new_pos_y;
            nearestVEdge(left, right, top, bottom, 
                stadium->players.pos_x[i], stadium->players.pos_y[i], &new_pos_x, &new_pos_y);
            new_pos_x -= stadium->playmode_side * OFFSIDE_KICK_MARGIN;
            stadium->players.pos_x[i] = new_pos_x;
            stadium->players.pos_y[i] = new_pos_y;
        }
    }
...
PUF_FN void offside_ref_callOffside(Stadium *stadium) {
    ...
    ref_placeBallAndChangePlayMode(stadium, PM_OffSide, stadium->offside_ref_last_kicker_side, 
        -stadium->offside_ref_last_kicker_side, pos_x, pos_y);
...
PUF_FN void offside_ref_setOffsideMark(Stadium *stadium, int kicker_index, const float accel_r) {
    ...
    if (stadium->playmode == PM_GoalKick
        || stadium->playmode == PM_KickIn
        || stadium->playmode == PM_CornerKick)
        return;
...
PUF_FN void free_kick_ref_placePlayersForGoalkick(Stadium *stadium) {
    Side oppside = -stadium->playmode_side;
    float p_area_left = stadium->playmode_side * (-PITCH_LENGTH / 2.0f + PENALTY_AREA_LENGTH / 2.0f) 
        - (PENALTY_AREA_LENGTH + BALL_SIZE * 2.0f) / 2.0f;
    float p_area_right = stadium->playmode_side * (-PITCH_LENGTH / 2.0f + PENALTY_AREA_LENGTH / 2.0f) 
        + (PENALTY_AREA_LENGTH + BALL_SIZE * 2.0f) / 2.0f;
    float p_area_top = (PENALTY_AREA_WIDTH + BALL_SIZE * 2.0f) / 2.0f;
    float p_area_bottom = -p_area_top;
    for (int i = 0 ; i < NUM_PLAYERS ; i++) {
        if (!stadium->players.enable[i]) continue;
...
PUF_FN void free_kick_ref_analyse(Stadium *stadium) {
    ...
    if (pm == PM_Back_Pass || pm == PM_CatchFault)
            ...
            if (stadium->free_kick_ref_kick_taken && PROPER_GOAL_KICKS) {
                if (r(stadium->ball_vel_x, stadium->ball_vel_y) < STOPPED_BALL_VEL) {
                    if (stadium->free_kick_ref_goal_kick_count >= MAX_GOAL_KICKS)
                        ref_awardFreeKick(stadium, -stadium->players.side[stadium->free_kick_ref_kick_taker_index], 
                            stadium->ball_pos_x, stadium->ball_pos_y);
                    else
                        ref_awardGoalKick(stadium, stadium->players.side[stadium->free_kick_ref_kick_taker_index], 
                            stadium->ball_pos_x, stadium->ball_pos_y);
                }
            }
            else {
...
PUF_FN void free_kick_ref_callFreeKickFault(Stadium *stadium, Side side, float x, float y) {
    ...
    stadium->ball_catcher_index = -1;
    ref_placeBallAndChangePlayMode(stadium, PM_Free_Kick_Fault, side, -side, x, y);
    stadium->free_kick_ref_after_free_kick_fault_time = 0;
}

PUF_FN void free_kick_ref_ballTouched(Stadium *stadium, int player_index) {
    if (stadium->playmode == PM_GoalKick && stadium->players.side[player_index] != stadium->playmode_side) {
        // opponent player kicks tha ball while ball is in penalty area.
        ref_awardGoalKick(stadium, -stadium->players.side[player_index], stadium->ball_pos_x, stadium->ball_pos_y);
...
PUF_FN void free_kick_ref_kickTaken(Stadium *stadium, int kicker_index, const float accel_r) {
    ...
    if (free_kick_ref_goalKick(stadium->playmode)) {
        if (stadium->playmode == PM_GoalKick && stadium->players.side[kicker_index] != stadium->playmode_side) {
    ...
    else if (stadium->playmode == PM_OffSide) {
        // do nothing
    }
...
PUF_FN bool touch_ref_checkGoal(Stadium *stadium) {
    if (stadium->playmode == PM_AfterGoal || stadium->playmode == PM_TimeOver)
        return false;
    ...
    if ((stadium->ball_catcher_index == -1 || stadium->players.side[stadium->ball_catcher_index] == LEFT)
        ...
        && !ref_isPenaltyShootOut(stadium, NEUTRAL)) {
        score(stadium, RIGHT);
        // mydiff
        ...
        if (HALF_TIME >= 0 && GOLDEN_GOAL && stadium->time >= HALF_TIME * NR_NORMAL_HALFS)
            ...
            changePlayMode(stadium, PM_AfterGoal, RIGHT);
        return true;
    }
        ...
        if (HALF_TIME >= 0 && GOLDEN_GOAL && stadium->time >= HALF_TIME * NR_NORMAL_HALFS)
            ...
            changePlayMode(stadium, PM_AfterGoal, LEFT);
        return true;
    }
...
PUF_FN void touch_ref_analyse(Stadium *stadium) {
    ...
    if (stadium->playmode == PM_AfterGoal) {
        if (++stadium->touch_ref_after_goal_time > AFTER_GOAL_WAIT) {
            ref_placeBallAndChangePlayMode(stadium, PM_KickOff, 
                (Side)-stadium->playmode_side, (Side)-stadium->playmode_side, 0.0f, 0.0f);
            ref_placePlayersInTheirField(stadium);
        }
        return;
    }
...
PUF_FN void catch_ref_callCatchFault(Stadium *stadium, Side side, float x, float y) {
    ...
    ref_moveOutOfPenalty(side, &x, &y);
    stadium->ball_catcher_index = -1;
    ref_placeBallAndChangePlayMode(stadium, PM_CatchFault, (Side)side, (Side)-side, x, y);
    stadium->catch_ref_after_catch_fault_time = 0;
}
...
PUF_FN void catch_ref_analyse(Stadium *stadium) {
    ...
    if (pm == PM_Back_Pass) {
        ...
        if (stadium->catch_ref_after_back_pass_time > AFTER_BACKPASS_WAIT)
            ...
            changePlayMode(stadium, PM_IndFreeKick, (Side)-stadium->playmode_side);
    ...
    if (pm == PM_CatchFault) {
        ...
        if (stadium->catch_ref_after_catch_fault_time > AFTER_CATCH_FAULT_WAIT)
            ...
            changePlayMode(stadium, PM_FreeKick, (Side)-stadium->playmode_side);

        return;
    }

    if (stadium->ball_catcher_index != -1 && pm != PM_AfterGoal && pm != PM_TimeOver
...
PUF_FN void foul_ref_playModeChange(Stadium *stadium, PlayMode pm) {
    if (pm == PM_Foul_Charge || pm == PM_Foul_Push)
...
PUF_FN void foul_ref_analyse(Stadium *stadium) {
    ...
    if (pm == PM_Foul_Charge || pm == PM_Foul_Push) {
        ...
        if (stadium->foul_ref_after_foul_time > AFTER_FOUL_WAIT) {
            if (ref_inPenaltyArea(stadium->playmode_side, stadium->ball_pos_x, stadium->ball_pos_y))
                changePlayMode(stadium, PM_IndFreeKick, (Side)-stadium->playmode_side);
            else
                changePlayMode(stadium, PM_FreeKick, (Side)-stadium->playmode_side);
        }
...
PUF_FN void foul_ref_callFoul(Stadium *stadium, int tackler_index) {
    ...
    stadium->ball_catcher_index = -1;
    ref_moveOutOfPenalty((Side)-stadium->players.side[tackler_index], &pos_x, &pos_y);
    ref_placeBallAndChangePlayMode(stadium, PM_Foul_Charge, (Side)stadium->players.side[tackler_index], 
        (Side)-stadium->players.side[tackler_index], pos_x, pos_y);
...
PUF_FN void penalty_ref_penalty_init(Stadium *stadium) {
    ...
    ref_placeBallAndChangePlayMode(stadium, PM_PenaltySetup, stadium->penalty_ref_cur_pen_taker, 
        NEUTRAL, -stadium->penalty_ref_pen_side * (PITCH_LENGTH / 2.0f - PEN_DIST_X), 0.0f);
}
...
PUF_FN bool penalty_ref_penalty_check_players(Stadium *stadium, const Side side ) {
    ...
    if (pm == PM_PenaltyMiss || pm == PM_PenaltyScore)
        return true;
    ...
    if (side == stadium->penalty_ref_cur_pen_taker) {
        ...
        if (iOutsideCircle == 0) {
            if (pm == PM_PenaltySetup) {
                if (distance(stadium->players.pos_x[goalie_index], stadium->players.pos_y[goalie_index], 
                    stadium->ball_pos_x, stadium->ball_pos_y) > 2.0f)
                {
                    // bCheck = false;
                }
                else
        ...
        else if (pm == PM_PenaltySetup && iOutsideCircle == 1)
            if (outside_player_index != -1 
                && distance(stadium->players.pos_x[outside_player_index], stadium->players.pos_y[outside_player_index], 
                    stadium->ball_pos_x, stadium->ball_pos_y) > 2.0f)
                bCheck = false;
    }
    else { //other team
        // goalie does not stand in front of goal line
        if (stadium->playmode != PM_PenaltyTaken)
    ...
    if (bCheck && outside_player_index != -1) {
        ...
        if ((side == LEFT && stadium->playmode == PM_PenaltySetup && stadium->playmode_side == LEFT
            ...
            || (side == RIGHT && stadium->playmode == PM_PenaltySetup && stadium->playmode_side == RIGHT
...
PUF_FN void penalty_ref_placeTakerTeamPlayers(Stadium *stadium) {
    const bool bPenTaken = stadium->playmode == PM_PenaltyTaken;
...
PUF_FN void penalty_ref_placeOtherTeamPlayers(Stadium *stadium) {
    const bool bPenTaken = stadium->playmode == PM_PenaltyTaken;
    const float goalie_line = stadium->penalty_ref_pen_side * (-PITCH_LENGTH/ 2.0f + PEN_MAX_GOALIE_DIST_X);
    ...
    for (int i = 0 ; i < NUM_PLAYERS ; i++) {
        ...
        if (stadium->players.goalie[i]) {
            if (!bPenTaken) {
                if (stadium->penalty_ref_pen_side * (stadium->players.pos_x[i] - goalie_line) > 0.0f) {
                    stadium->players.pos_x[i] = goalie_line - stadium->penalty_ref_pen_side * 1.5f;
            ...
            if (!(distance(0.0f, 0.0f, stadium->players.pos_x[i], stadium->players.pos_y[i]) <= c_radius)) {
                ...
                nearestEdgeCircle(0.0f, 0.0f, c_radius, 
                    stadium->players.pos_x[i], stadium->players.pos_y[i], &stadium->players.pos_x[i], &stadium->players.pos_y[i]);
            }
        }
    }
}
...
PUF_FN void penalty_ref_penalty_score(Stadium *stadium, Side side) {
    changePlayMode(stadium, PM_PenaltyScore, side);
    penaltyScore(stadium, side, true );
...
PUF_FN void penalty_ref_penalty_miss(Stadium *stadium, Side side) {
    changePlayMode(stadium, PM_PenaltyMiss, side);
    stadium->penalty_ref_pen_nr_taken++;
    penaltyScore(stadium, side, false);
    penalty_ref_penalty_check_score(stadium);
}
...
PUF_FN void penalty_ref_penalty_foul(Stadium *stadium, const Side side) {
    ...
    if (side != NEUTRAL && side == stadium->penalty_ref_cur_pen_taker)
        penalty_ref_penalty_miss(stadium, side);
    else
        penalty_ref_penalty_score(stadium, -side);
}
...
PUF_FN void penalty_ref_handleTimeout(Stadium *stadium, bool left_move_check, bool right_move_check) {
    ...
    if (PEN_COACH_MOVES_PLAYERS && pm == PM_PenaltySetup) {
        if (!left_move_check)
    ...
    if (pm == PM_PenaltyMiss || pm == PM_PenaltyScore)
        ...
        if (pm == PM_PenaltySetup)
            changePlayMode(stadium, PM_PenaltyReady, stadium->playmode_side);
        // time elapsed -> missed goal
        else if (pm == PM_PenaltyTaken || pm == PM_PenaltyReady)
            penalty_ref_penalty_miss(stadium, stadium->playmode_side);
    }
...
PUF_FN void penalty_ref_handleTimer(Stadium *stadium, const bool left_move_check, const bool right_move_check) {
    ...
    if (pm == PM_PenaltyScore || pm == PM_PenaltyMiss) {
        // freeze the ball
    ...
    if (left_move_check && right_move_check) {
        ...
        if (ref_crossGoalLine(stadium, stadium->penalty_ref_pen_side, 
            stadium->penalty_ref_prev_ball_pos_x, stadium->penalty_ref_prev_ball_pos_y)) {
            if (pm == PM_PenaltyTaken)
                penalty_ref_penalty_score(stadium, stadium->playmode_side);
        ...
        else if (fabs(stadium->ball_pos_x) > PITCH_LENGTH * 0.5f + BALL_SIZE 
            || fabs(stadium->ball_pos_y) > PITCH_WIDTH * 0.5f + BALL_SIZE) {
            placeBall(stadium, stadium->penalty_ref_pen_side, stadium->ball_pos_x, stadium->ball_pos_y);
            if (pm == PM_PenaltyTaken)
                penalty_ref_penalty_miss(stadium, stadium->playmode_side);
        }
    ...
    else if (pm == PM_PenaltyReady || pm == PM_PenaltyTaken) {
...
PUF_FN void penalty_ref_analyse(Stadium *stadium) {
    ...
    if (PEN_COACH_MOVES_PLAYERS && (pm == PM_PenaltyReady || pm == PM_PenaltyTaken)) {
        if (!bCheckLeft)
            penalty_ref_penalty_place_all_players(stadium, LEFT);
...
PUF_FN void penalty_ref_kickTaken(Stadium *stadium, int kicker_index, const float accel_r) {
    ...
    if (stadium->playmode == PM_PenaltySetup)
    ...
    // cannot kick second time after penalty was taken
        ...
        && stadium->playmode == PM_PenaltyTaken
    ...
    else if (stadium->playmode == PM_PenaltyReady || stadium->playmode == PM_PenaltyTaken) {
        if (stadium->playmode == PM_PenaltyReady && stadium->players.side[kicker_index] == stadium->penalty_ref_cur_pen_taker
    ...
    if (stadium->playmode == PM_PenaltyReady && stadium->playmode_side == LEFT) {
        ...
        changePlayMode(stadium, PM_PenaltyTaken, LEFT);
    }
    else if (stadium->playmode == PM_PenaltyReady && stadium->playmode_side == RIGHT) {
        ...
        changePlayMode(stadium, PM_PenaltyTaken, RIGHT);
    }
    // if it was not allowed to kick, don't move ball
    else if (stadium->playmode != PM_PenaltyTaken)
        placeBall(stadium, stadium->penalty_ref_pen_side, stadium->ball_pos_x, stadium->ball_pos_y);
}
...
PUF_FN void stadium_step(Stadium *stadium) {
    ...
    else if (stadium->playmode == PM_AfterGoal
        || stadium->playmode == PM_OffSide
        || stadium->playmode == PM_Illegal_Defense
        || stadium->playmode == PM_Foul_Charge
        || stadium->playmode == PM_Foul_Push
...
PUF_FN void stadium_init(Stadium *stadium, unsigned int seed, unsigned int left_players_count, unsigned int right_players_count) {
    assert(left_players_count + right_players_count == NUM_PLAYERS 
        && "left players count + right players count should be equal to NUM_PLAYERS");
    ...
    stadium->team_right_enabled = left_players_count > 0;
}
