...
#ifndef TIME_REFEREE_ENABLED
#define TIME_REFEREE_ENABLED 1
#endif

#ifndef BALL_STUCK_REFEREE_ENABLED
#define BALL_STUCK_REFEREE_ENABLED 1
#endif

#ifndef OFFSIDE_REFEREE_ENABLED
#define OFFSIDE_REFEREE_ENABLED 1
#endif

#ifndef FREE_KICK_REFEREE_ENABLED
#define FREE_KICK_REFEREE_ENABLED 1
#endif

#ifndef TOUCH_REFEREE_ENABLED
#define TOUCH_REFEREE_ENABLED 1
#endif

#ifndef CATCH_REFEREE_ENABLED
#define CATCH_REFEREE_ENABLED 1
#endif

#ifndef FOUL_REFEREE_ENABLED
#define FOUL_REFEREE_ENABLED 1
#endif

#ifndef PENALTY_REFEREE_ENABLED
#define PENALTY_REFEREE_ENABLED 1
#endif

#if TIME_REFEREE_ENABLED || BALL_STUCK_REFEREE_ENABLED || OFFSIDE_REFEREE_ENABLED || FREE_KICK_REFEREE_ENABLED || TOUCH_REFEREE_ENABLED || CATCH_REFEREE_ENABLED || FOUL_REFEREE_ENABLED || PENALTY_REFEREE_ENABLED
...
typedef struct {
    Side side[NUM_PLAYERS];
    bool goalie[NUM_PLAYERS];
    int unum[NUM_PLAYERS];
    float size[NUM_PLAYERS];
    float pos_x[NUM_PLAYERS];
    float pos_y[NUM_PLAYERS];
    float vel_x[NUM_PLAYERS];
    float vel_y[NUM_PLAYERS];
    float accel_x[NUM_PLAYERS];
    float accel_y[NUM_PLAYERS];
    float max_speed[NUM_PLAYERS];
    float max_accel[NUM_PLAYERS];
    float decay[NUM_PLAYERS];
    float randp[NUM_PLAYERS];
    float post_collision_pos_x[NUM_PLAYERS];
    float post_collision_pos_y[NUM_PLAYERS];
    int collision_count[NUM_PLAYERS];
    bool collided[NUM_PLAYERS];
    float kick_rand[NUM_PLAYERS];

    float stamina[NUM_PLAYERS];
    float effort[NUM_PLAYERS];
    float recovery[NUM_PLAYERS];
    float stamina_capacity[NUM_PLAYERS];
    float consumed_stamina[NUM_PLAYERS];

    float angle_body[NUM_PLAYERS];
    float angle_body_committed[NUM_PLAYERS];
    float angle_neck[NUM_PLAYERS];
    float angle_neck_committed[NUM_PLAYERS];
    
    bool enable[NUM_PLAYERS];
    int32_t state[NUM_PLAYERS];
    bool ball_collide[NUM_PLAYERS];
    bool player_collide[NUM_PLAYERS];
    bool post_collide[NUM_PLAYERS];

    bool command_done[NUM_PLAYERS];
    bool turn_neck_done[NUM_PLAYERS];
    bool done_received[NUM_PLAYERS];

    CommandType left_leg_command_type[NUM_PLAYERS];
    CommandType right_leg_command_type[NUM_PLAYERS];
    float left_leg_dash_power[NUM_PLAYERS];
    float right_leg_dash_power[NUM_PLAYERS];
    float left_leg_dash_dir[NUM_PLAYERS];
    float right_leg_dash_dir[NUM_PLAYERS];

    int kick_cycles[NUM_PLAYERS];
    int dash_cycles[NUM_PLAYERS];
    int tackle_cycles[NUM_PLAYERS];
    int foul_cycles[NUM_PLAYERS];

    int kick_count[NUM_PLAYERS];
    int dash_count[NUM_PLAYERS];
    int tackle_count[NUM_PLAYERS];
    int foul_count[NUM_PLAYERS];
    int turn_count[NUM_PLAYERS];
    // int catch_count[NUM_PLAYERS];
    // int move_count[NUM_PLAYERS];
    int turn_neck_count[NUM_PLAYERS];
    // int change_focus_count[NUM_PLAYERS];
    // int change_view_count[NUM_PLAYERS];
    // int say_count[NUM_PLAYERS];
    int card_count[NUM_PLAYERS];

    MyHeteroPlayer player_type[NUM_PLAYERS];

    int hear_capacity_from_teammate[NUM_PLAYERS];
    int hear_capacity_from_opponent[NUM_PLAYERS];
    int goalie_catch_ban[NUM_PLAYERS];
} Players;
...
typedef struct {
    ...
    float ball_max_accel;
    float ball_decay;
    float ball_randp;
    float ball_post_collision_pos_x;
    float ball_post_collision_pos_y;
    int ball_collision_count;
    bool ball_collided;
    ...
    float ball_stuck_ref_last_ball_pos_x;
    float ball_stuck_ref_last_ball_pos_y;
    ...
    float offside_ref_offside_pos_x;
    float offside_ref_offside_pos_y;
    int offside_ref_offside_candidates_player_index[NUM_PLAYERS];
    float offside_ref_offside_candidates_pos_x[NUM_PLAYERS];
    float offside_ref_offside_candidates_pos_y[NUM_PLAYERS];
    ...
    float touch_ref_prev_ball_pos_x;
    float touch_ref_prev_ball_pos_y;
    ...
    float penalty_ref_prev_ball_pos_x;
    float penalty_ref_prev_ball_pos_y;
