#ifndef PUFFER_RCSSFULL_H
#define PUFFER_RCSSFULL_H
...
#define OBS_SIZE (OBS_SELF_INDEX_STRIDE + OBS_TIME_STRIDE + OBS_TASK_STRIDE + OBS_BALL_STRIDE + OBS_SELF_STAMINA + REWARD_STRIDE \
    + NUM_PLAYERS * OBS_PLAYER_STRIDE + NUM_OPPONENTS * OBS_PLAYER_STRIDE)
...
typedef Env RcssFull;

void puf_close(RcssFull* env);

static void configure_stadium_paths(RcssFull* env) {
...
static void place_ball(RcssFull* env, float* ball_x, float* ball_y, float x0, float x1, float y_half) {
...
static void place_player(RcssFull* env, int player_index, float ball_x, float ball_y, float x0, float x1, float y_half) {
...
static void ensure_play_on(RcssFull* env) {
...
static void clear_observations(RcssFull* env) {
    for (int a = 0; a < env->num_agents; a++)
        memset(env->agents[a].observations, 0, (size_t)OBS_SIZE * sizeof(obs_t));
}

static float goal_event_reward(RcssFull* env, int scored_side) {
...
static float fault_event_reward(RcssFull* env, int fault_side) {
...
static void apply_agent_rewards(RcssFull* env, Reward r, int terminal) {
    for (int a = 0; a < env->num_agents; a++) {
...
static int goal_diff(RcssFull* env) {
...
static int episode_success(RcssFull* env) {
    return goal_diff(env) > 0;
}

static float remaining_time_frac(const RcssFull* env) {
...
static int handle_early_terminal(RcssFull* env, PlayMode pm) {
    Reward r = {0};
...
static float score_early_terminal(RcssFull* env) {
...
static float perf_early_terminal(RcssFull* env) {
    if (!episode_success(env))
        return 0.0f;
...
static int fixed_horizon_ticks(RcssFull* env) {
...
static int handle_fixed_horizon(RcssFull* env, PlayMode pm) {
...
static float score_fixed_horizon(RcssFull* env) {
...
static float perf_fixed_horizon(RcssFull* env) {
    int goals_for = (env->side == SIDE_LEFT)
        ? env->episode_left_goal : env->episode_right_goal;
    if (goals_for <= 0)
        return 0.0f;
...
void add_player_log(RcssFull* env, int player_index) {
...
static void update_difficulty(RcssFull* env) {
...
void add_log(RcssFull* env) {
    for (int i = 0; i < env->num_agents; i++)
        add_player_log(env, i);
}

void handle_rewards_and_terminals(RcssFull* env) {
...
static void pack_time_obs(RcssFull* env, float *obs, int *idx) {
    obs[(*idx)++] = (float)env->tick / env->starting_horizon_ticks;
}

static void pack_task_obs(RcssFull* env, int a, float *obs, int *idx) {
...
static void compute_observations(RcssFull* env) {
    for (int a = 0; a < env->num_agents; a++) {
