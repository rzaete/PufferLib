...
int invictus_trainer_send_action(InvictusTrainer* t, bool should_log);
...
static int invictus_trainer_send_init(InvictusTrainer* t)
{
    ...
    for (int k = 0; k < 6; k++) {
        int n = invictus_transport_send(&t->comm, buf, false);
        if (n > 0) return 0;
...
static int invictus_send_trainer_commands_split(InvictusTransport *comm, const char *msg, bool should_log)
{
    ...
    for (;;) {
        ...
        if (len > 0) {
            ...
            tmp[len] = '\0';
            if (invictus_transport_send(comm, tmp, should_log) > 0) ++count;
        }
        if (!*p) break;
    }
    return count;
...
int invictus_trainer_send_action(InvictusTrainer* t, bool should_log)
{
    ...
    if (rc == 0 && buf[0]) {
        ...
        if (should_log)
            invictus_log("cycle %d sending %s", t->current_cycle, buf);
        (void)invictus_send_trainer_commands_split(&t->comm, buf, should_log);
    }
...
int invictus_trainer_close(InvictusTrainer* t)
{
    ...
    if (looks_live) {
        if (t->server_alive) {
            int n = invictus_transport_send(&t->comm, "(bye)", false);
            if (n <= 0) {
